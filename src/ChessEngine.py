class GameState:  # code trang thai tro choi 
    def __init__ (self):
        ...
        # Thêm các biến mới
        self.halfmove_clock = 0  # Đếm cho luật 50 nước
        self.position_counter = {}  # Đếm lặp lại vị trí
    ...
    def makeMove(self, move, piecePromotion = "Q"):
        ...
        if move.is_enpassant_move:
            # Kiểm tra xem nước đi có đúng là en passant không
            captured_pawn_row = move.end_row + 1 if move.piece_moved[0] == 'w' else move.end_row - 1
            captured_pawn = self.board[captured_pawn_row][move.end_col]
            
            # Chỉ xóa quân khi đúng là quân tốt và có màu đối lập
            if captured_pawn[1] == 'p' and captured_pawn[0] != move.piece_moved[0]:
                self.board[captured_pawn_row][move.end_col] = "--"
                move.piece_captured = captured_pawn  # Đảm bảo quân bị bắt được ghi nhận đúng
        ...
        # Cập nhật halfmove_clock cho luật 50 nước
        ...
        # Cập nhật position_counter cho threefold repetition
        position_key = self.getBoardPositionKey()
        ...
        self.checkInsufficientMaterial()
    
    def getBoardPositionKey(self):
        """Tạo khóa duy nhất cho vị trí hiện tại"""
        position = []
        for row in self.board:
            position.append(''.join(row))
        position.append('w' if self.white_to_move else 'b')
        
        # Thêm thông tin về nhập thành và en passant
        castling = ''
        if self.current_castling_rights.wks: castling += 'K'
        if self.current_castling_rights.wqs: castling += 'Q'
        if self.current_castling_rights.bks: castling += 'k'
        if self.current_castling_rights.bqs: castling += 'q'
        position.append(castling if castling else '-')
        
        if self.enpassant_possible:
            ep_square = chr(97 + self.enpassant_possible[1]) + str(8 - self.enpassant_possible[0])
            position.append(ep_square)
        else:
            position.append('-')
    ...
    def checkInsufficientMaterial(self):
        """Kiểm tra điều kiện không đủ quân chiếu hết"""
        pieces = {}
        for row in range(8):
            for col in range(8):
        ...
        # Các trường hợp không đủ quân
        if len(pieces) == 2:  # Chỉ còn 2 vua
            self.insufficient_material = True
        elif len(pieces) == 3 and any(p[1] in ['B', 'N'] for p in pieces):  # Vua + mã/tượng
            self.insufficient_material = True
    ...
    def undoMove(self):
        if len(self.move_log) != 0: # hoan tac lai di chuyen
            ...
            if move.is_enpassant_move:
                # Để ô đích trống vì đó không phải là nơi quân bị bắt
                self.board[move.end_row][move.end_col] = "--"
                
                # Đặt lại tốt bị bắt vào đúng vị trí
                # Quân tốt bị bắt nằm cùng hàng với quân tốt thực hiện bắt nhưng ở cột khác
                captured_pawn_row = move.end_row + 1 if move.piece_moved[0] == 'w' else move.end_row - 1
                self.board[captured_pawn_row][move.end_col] = move.piece_captured
    ...
    def getValidMoves(self):
        ...
        if len(moves) == 0:
            ...
            # Check all possible draw conditions
            if self.halfmove_clock >= 100:
                self.fifty_move_rule = True
            if self.position_counter.get(self.getBoardPositionKey(), 0) >= 3:
                self.threefold_repetition = True
            self.checkInsufficientMaterial()
    ...
    def getKingMoves(self, row, col, moves):
        ...
        for i in range(8):
            ...
            if 0 <= end_row <= 7 and 0 <= end_col <= 7:
                # King can never be pinned, so we just check if the destination is valid
                end_piece = self.board[end_row][end_col]
...
class Move:
    ...
    def __init__(self, start_square, end_square, board, is_enpassant_move=False, is_castle_move=False):
        ...
        if self.is_enpassant_move:
            # Ensure that the captured piece is properly set for en passant
            # For white pawn moving diagonally up, captured piece is a black pawn
            # For black pawn moving diagonally down, captured piece is a white pawn
